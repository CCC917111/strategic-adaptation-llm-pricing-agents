"""Run one cell of the completed paper-based static-market baseline.

The defaults reproduce the archived implementation: a static calibrated
market, fixed regulatory benchmark, 100 rounds, and the paper response order.
The module intentionally calls this a paper-based baseline rather than an
exact replication; see ``experiments/paper-baseline-design.json``.
"""
...
async def run(args: argparse.Namespace) -> None:
    ...
    mode = OversightMode(args.mode)
    config = {
        "experiment": "paper-based-baseline-reproduction",
        "provider": "gemini",
        "model": args.model,
        ...
        "market": "static_calibrated_logit",
        "regulatory_benchmark": (
            "round_best_response"
            if args.round_best_response
            else "fixed_symmetric_nash_1.473"
        ),
    ...
    runner = GameRunner(
        agents=create_agent_pair(
            ...
            prompts=PromptFactory(
                PromptConfig(response_order=response_order)
            ),
        ),
        market=LogitMarket(),
        regulator=Regulator(mode),
        use_round_best_response_benchmark=args.round_best_response,
    )
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument(
        "--api-method",
        choices=("interactions", "generate_content", "auto"),
    ...
    result.add_argument(
        "--response-order",
        choices=tuple(order.value for order in ResponseOrder),
        default="paper",
    )
    ...
    result.add_argument("--round-delay", type=float, default=15.0)
    result.add_argument("--max-attempts", type=int, default=5)
    result.add_argument("--base-retry-delay", type=float, default=2.0)
    result.add_argument("--early-stop", action="store_true")
    result.add_argument("--round-best-response", action="store_true")
    return result
